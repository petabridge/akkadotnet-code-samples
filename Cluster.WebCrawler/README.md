...
2. There can only be one `DownloadsTracker` ([source](src/WebCrawler.TrackingService/Actors/Tracking/DownloadsTracker.cs)) actor per `CrawlJob`, because figuring out which documents are new and which ones aren't requires a strong level of consistency.
